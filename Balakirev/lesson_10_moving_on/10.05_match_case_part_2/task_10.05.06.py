""" Подвиг 5. Вводятся данные по книге в виде строки через запятую в формате
...
book = [t[i](x) if t[i] != str else x.strip() for i, x in
...
t = (int, str, str, float, int)
book = [t[i](v) for i, v in enumerate(input().split(", "))]
match book:
    case [int() as number, str() as author, str() as title] if len(
            author) >= 6 and len(title) >= 10:
        print('Yes')
    case [int() as number, str() as author, str() as title,
          float() as price] if len(author) >= 6 and price > 0:
        print('Yes')
    case [int() as number, str() as author, str() as title,
          int() as year] if year >= 2020:
        print('Yes')
    case [int() as number, str() as author, str() as title, float() as price,
          int() as year] if price > 0 and year >= 2020:
        print('Yes')
    case _:
        print('No')
