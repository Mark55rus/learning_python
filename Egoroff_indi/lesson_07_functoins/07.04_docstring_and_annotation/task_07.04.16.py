""" Шифр цезаря.
    На основании предыдущей задачи мы с вами можем реализовать знаменитый шифр Цезаря. Этот шифр брал каждую букву
исходной фразы и смещал ее на значение ключа, это так раз был на сдвиг. В пределах кодирования одной фразы значение
сдвига всегда постоянно.
...
def shift_letter(symbol: str, shift: int) -> str:
    """Функция сдвигает символ letter на shift позиций"""
    s = chr((ord(symbol) - 97 + shift) % 26 + 97)
    return s
...
def caesar_cipher(text: str, shift: int) -> str:
    ...
    for symbol in text:
        if symbol.isalpha():
            cipher += shift_letter(symbol, shift)
        else:
            cipher += symbol
    return cipher
